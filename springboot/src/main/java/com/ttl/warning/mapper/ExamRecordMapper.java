...
public interface ExamRecordMapper {
    ...
    @Select("select r.id, r.score, r.is_passed, r.submit_time, u.real_name as studentName from student_exam_record r join users u on r.student_id=u.id where r.exam_id=#{examId} order by r.submit_time desc")
    List<Map<String, Object>> findScoresByExam(Long examId);

    @Select("select a.*, q.content, q.correct_option from student_answer_record a join question_bank q on a.question_id=q.id where a.exam_record_id=#{recordId}")
    List<Map<String, Object>> findAnswerDetails(Long recordId);

    @Select("select avg(score) from student_exam_record")
    Double averageScore();

    @Select("select sum(case when is_passed=1 then 1 else 0 end)*1.0/count(*) from student_exam_record")
    Double passRate();
    ...
    @Select("select * from student_exam_record where student_id=#{studentId}")
